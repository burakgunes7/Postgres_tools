...
if [ $node2_master -eq 1 ] && [ $IS_NODE1_DOWN == 0 ]; then
    ...
    create_folder=$(ssh root@192.168.218.135 mkdir -p /mnt/volume1/postgresql/12/main/)
    if [ $? -eq 0 ]; then
    ...
    owner=$(ssh root@192.168.218.135 chown -R postgres:postgres /mnt/)

    backup=$(
        ssh postgres@192.168.218.135 pg_basebackup -h 192.168.218.136 -D /mnt/volume1/postgresql/12/main/ -U replicator -P -v -R -X stream -C -S slaveslot1
    )
    if [ $? -eq 0 ]; then
    ...
    permission=$(ssh root@192.168.218.135 sudo chmod 700 -R /mnt/volume1/postgresql/12/main/)

    conf_reload=$(ssh root@192.168.218.135 pg_ctlcluster 12 main reload)
fi
...
if [ $node1_master -eq 1 ] && [ $IS_NODE1_DOWN == 0 ]; then
    ...
    #     echo "[ $date ] POSTGRES DROP REPLICASLOT FAIL"
    # fi
    remove=$(ssh root@192.168.218.136 sudo rm -rf /mnt/volume1)
    if [ $? -eq 0 ]; then
        echo "[ $date ] POSTGRES REMOVE FOLDER OK"
    else
        echo "[ $date ]  POSTGRES REMOVE FOLDER FAIL"
    ...
    create_folder=$(ssh root@192.168.218.136 mkdir -p /mnt/volume1/postgresql/12/main/)
    if [ $? -eq 0 ]; then
        echo "[ $date ] POSTGRES CREATE FOLDER OK"
    else
        echo "[ $date ] POSTGRES CREATE FOLDER FAIL"
    ...
    owner=$(ssh root@192.168.218.136 chown -R postgres:postgres /mnt/)

    backup=$(
        ssh postgres@192.168.218.136 pg_basebackup -h 192.168.218.135 -D /mnt/volume1/postgresql/12/main/ -U replicator -P -v -R -X stream -C -S slaveslot1
    )
    if [ $? -eq 0 ]; then
        echo "[ $date ] POSTGRES BACKUP SLAVESLOT1 OK"
    else
    ...
    permission=$(ssh root@192.168.218.136 sudo chmod 700 -R /mnt/volume1/postgresql/12/main/)

    conf_reload=$(ssh root@192.168.218.136 pg_ctlcluster 12 main reload)
fi
